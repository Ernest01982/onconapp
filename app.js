...
const cycleStateLabel = state => state === 'review' ? 'Needs review — set next cycle' : state === 'open' ? 'Listing window open' : state === 'due' ? 'Approaching listing window' : 'Scheduled listing window';
...
function homeView() {
  ...
  const listingReview = listingReminders.filter(item=>item.state==='review');
  ...
  <main class="content">
    ...
    ${listingReview.length?`<button class="btn btn-secondary btn-block" style="margin-top:14px" data-screen="activity" data-filter="cycles">${listingReview.length} listing ${listingReview.length===1?'window needs':'windows need'} review · Set next cycle</button>`:''}
  </main>${nav()}`;
}
...
function listingCycleCard(reminder) {
  ...
  const review=reminder.state==='review';
  const tone=review?'inactive':reminder.state==='open'?'listed':reminder.state==='due'?'hot':'';
  return `<article class="card cycle-card"><div class="cycle-card-main"><span class="status ${tone}">${cycleStateLabel(reminder.state)}</span><h3>${esc(c?.name||'Client')}</h3><p>${review?`Previous window ${shortDate(reminder.targetAt)} — confirm the next buying date.`:reminder.state==='open'?`Window opened ${dayLabel(reminder.targetAt)}`:`Target ${dayLabel(reminder.targetAt)}`} · ${reminder.leadDays}-day reminder${c?.contact?` · ${esc(c.contact)}`:''}</p>${c?.listingCycleNotes?`<small>${esc(c.listingCycleNotes)}</small>`:''}</div><button class="mini-btn" data-action="customer" data-id="${esc(reminder.customerId)}">${review?'Review cycle':'Open'}</button></article>`;
}
...
function reportsView() {
  ...
  const {visits,tasksDue,tasksCompleted,trips,minutes:mins,undatedCompleted}=activityMetrics(data,range);
  ...
  const conversion=interestedEver.length?`${Math.round(converted/interestedEver.length*100)}%`:'—';
  ...
  return `${topbar('Reports','Management snapshot')}<main class="content">${rangeControls('report',reportPeriod,reportCustomStart,reportCustomEnd)}<div class="section-row"><h2>Activity</h2><span class="status">Selected period</span></div><p class="modal-hint">${esc(range.label)} · Visit totals use the visit start date; completions use the actual completion date.</p><div class="metric-grid"><div class="card metric"><span>Visits</span><strong>${visits.length}</strong></div><div class="card metric"><span>Accounts visited</span><strong>${new Set(visits.map(v=>v.customerId)).size}</strong></div><div class="card metric"><span>Time in trade</span><strong>${formatTradeTime(mins)}</strong></div><div class="card metric"><span>New accounts</span><strong>${newAccounts}</strong></div><div class="card metric"><span>Open follow-ups due</span><strong>${tasksDue.filter(t=>!t.done).length}</strong></div><div class="card metric"><span>Completed in period</span><strong>${tasksCompleted.length}</strong></div></div>${undatedCompleted?`<p class="modal-hint" role="status">Across all history, ${undatedCompleted} completed follow-up(s) have no valid completion date and are excluded from period completion totals. Their records are unchanged.</p>`:''}
  <div class="section-row"><h2>Menu / listing cycles</h2><span class="status hot">Current</span></div><div class="metric-grid"><div class="card metric"><span>Windows open</span><strong>${listingWindows.filter(item=>item.state==='open').length}</strong></div><div class="card metric"><span>Approaching</span><strong>${listingWindows.filter(item=>item.state==='due').length}</strong></div><div class="card metric"><span>Scheduled later</span><strong>${listingWindows.filter(item=>item.state==='upcoming').length}</strong></div><div class="card metric"><span>Needs review</span><strong>${listingWindows.filter(item=>item.state==='review').length}</strong></div></div>
  <div class="section-row"><h2>Wine pipeline</h2><span class="status">Current</span></div><div class="metric-grid"><div class="card metric"><span>Interested</span><strong>${pipeline.filter(r=>r.status==='Interested').length}</strong></div><div class="card metric"><span>Sampled</span><strong>${pipeline.filter(r=>r.status==='Sampled').length}</strong></div><div class="card metric"><span>Considering</span><strong>${pipeline.filter(r=>r.status==='Considering').length}</strong></div><div class="card metric"><span>Ever listed · all history</span><strong>${conversion}</strong></div></div><p class="modal-hint">Pipeline counts show current status. Ever listed is the share of relationships ever recorded in the pipeline that were also recorded as Listed, across all saved history; it is not a selected-period conversion rate.</p>
  <div class="section-row"><h2>Listings</h2><span class="status listed">Confirmed only</span></div><div class="metric-grid"><div class="card metric"><span>Active listings · current</span><strong>${listed.length}</strong></div><div class="card metric"><span>Listed in period</span><strong>${newListings.length}</strong></div><div class="card metric"><span>Delisted in period</span><strong>${delistings.length}</strong></div><div class="card metric"><span>Products discussed · period</span><strong>${discussed.length}</strong></div></div><div class="list report-breakdowns">${breakdown('Current listings by wine',listingByWine,'No confirmed listings yet.')}${breakdown('Current listings by restaurant',listingByCustomer,'No confirmed listings yet.')}</div>
  <div class="section-row"><h2>Travel</h2><span class="status">Selected period</span></div><div class="metric-grid"><div class="card metric"><span>Business KM</span><strong>${km.toFixed(1)}</strong></div><div class="card metric"><span>Trips</span><strong>${trips.length}</strong></div><div class="card metric"><span>KM by accounts</span><strong>${new Set(trips.map(t=>t.customerId).filter(Boolean)).size}</strong></div><div class="card metric"><span>Mileage claim</span><strong>${currency(claim)}</strong></div></div>
  <div class="section-row"><h2>Summary & current priorities</h2></div><div class="card insight">${icon('spark','icon-lg')}<p><strong>${new Set(visits.map(v=>v.customerId)).size} customers reached.</strong> ${pendingFollowUps().length?`${pendingFollowUps().length} open follow-ups currently need attention.`:'All committed follow-ups are on track.'} ${actionableWindows.length?`${actionableWindows.length} venues are in or approaching a listing window.`:''} Only confirmed Listed relationships are counted as active placements. Listed/delisted period totals count each venue–wine relationship once per status, including relistings.</p></div><div class="card info-card" style="margin-top:10px"><h3>Products discussed</h3><p class="muted-copy">${discussed.length?discussed.map(esc).join(' · '):'No products logged in this period.'}</p></div><button class="btn btn-secondary btn-block" style="margin-top:14px" data-action="share-report">${icon('mail')} Email this summary</button></main>${nav()}`;
}
...
function customerFormModal(customerId = null, fromVisit = false) {
  ...
  modal=`<div class="modal-backdrop" data-action="close-modal"><form class="modal" id="customer-form" data-id="${existing?.id||''}">
    ...
    <label class="location-option"><input type="checkbox" name="savePhoneContact" ${item.savePhoneContact==='on'?'checked':''}><span><span><strong>Also save to phone contacts</strong><small>Get a contact file with the venue, person, phone and email. Open it in Contacts and confirm Save. Existing phone contacts are not automatically updated.</small></span></span></label>
...
function answerQuestion(q) {
  ...
  const lower=q.toLowerCase(); const pending=data.tasks.filter(t=>!t.done&&(!scope.customer||t.customerId===scope.customer.id)).sort((a,b)=>new Date(a.due)-new Date(b.due));const windows=cycleReminders().filter(w=>!scope.customer||w.customerId===scope.customer.id),priorityWindows=windows.filter(item=>item.state==='open'||item.state==='due');
  ...
  if (/menu|listing window|buying window|reopen/.test(lower)) { const review=windows.filter(item=>item.state==='review'),first=priorityWindows[0]||windows.find(item=>item.state==='upcoming');const message=first?`${priorityWindows.length} venues are in or approaching a listing window. ${first.state==='upcoming'?'Next scheduled venue':'Start with'}: ${customer(first.customerId)?.name}; target ${dayLabel(first.targetAt)}, ${first.leadDays}-day reminder.`:'No open, approaching or future listing windows are scheduled.';return message+(review.length?` ${review.length} expired window(s) need review. Confirm the next buying date for ${customer(review[0].customerId)?.name}.`:''); }
...
function phoneContactModal(id) {
  const c=customer(id);if(!c)return;
  modal='<div class="modal-backdrop" data-action="close-modal"><section class="modal" role="dialog" aria-modal="true" aria-label="Save to phone contacts">'
    +'<div class="modal-head"><h2>Save to phone contacts</h2><button class="close-btn" data-action="close-modal" aria-label="Close">'+icon('x')+'</button></div>'
    +'<div class="card info-card"><h3>'+esc(c.contact||c.name)+'</h3><p>'+esc(c.name)+'</p><p>'+esc(c.phone||'No phone number saved')+'</p><p>'+esc(c.email||'No email saved')+'</p></div>'
    +'<p class="modal-hint">Download the contact file, open it from Downloads, choose Contacts and confirm Save. Only the venue, contact person, role, phone and email are included—not your private notes.</p>'
    +'<p class="modal-hint">This is a one-off copy, not automatic contact syncing. Check for an existing phone contact before saving again to avoid duplicates.</p>'
    +'<button class="btn btn-primary btn-block" data-action="download-phone-contact" data-id="'+esc(c.id)+'">Download contact file</button>'
    +'<p id="phone-contact-status" class="modal-hint" role="status">Your FieldFlow client stays saved if you cancel the phone import.</p></section></div>';
  render();
}
...
document.addEventListener('click', async event => {
  ...
  if(action==='close-modal'){
  ...
  else if(action==='unknown-listing-followup'){
    if(!planListingDateFollowUp(data.activeVisit)){toast('A listing date is already saved. Review it in Menu / listing cycle.');return;}
    save();render();const field=document.querySelector('[data-visit-field="followUpAt"]');field?.scrollIntoView({block:'center'});field?.focus();toast('Choose when to ask again. Existing follow-up details are kept.');
  }
    ...
    if(active.followUpRequired&&(!active.followUpAt||!active.followUpReason)){toast('Add the follow-up date and reason, or choose No.');return;}
  ...
  else if(action==='phone-contact')phoneContactModal(target.dataset.id);
  else if(action==='download-phone-contact'){
    const status=document.getElementById('phone-contact-status');
    try{const file=buildPhoneContact(customer(target.dataset.id));downloadFile(file.filename,file.content,file.type);if(status)status.textContent='Contact file download started. Open the .vcf file from Downloads, choose Contacts and confirm Save. Your client is already saved in FieldFlow.';}
    catch(error){if(status)status.textContent=error.message||'Contact download could not start. Your client is still saved; please retry.';}
  }
    ...
    const buttons=document.querySelectorAll('[data-action="share-pdf"],[data-action="share-pdf-whatsapp"]');
    buttons.forEach(button=>button.disabled=true);
    const result=await sharePriceListPdf(priceListPdfFile);
    buttons.forEach(button=>button.disabled=!canSharePdf(priceListPdfFile));
    if(result==='shared')toast('PDF handed to your chosen app. Review the recipient and send there.');
    else if(result==='failed'||result==='unsupported')toast('File sharing is unavailable. Download the PDF and attach it in WhatsApp or email.');
  }
  else if(action==='copy-pdf-phone'){
    const input=document.getElementById('pdf-recipient-phone');if(!input?.value)return;
    try{await navigator.clipboard.writeText(input.value);toast('Client phone number copied');}
    catch{input.focus();input.select();toast('Press and hold the selected number to copy it.');}
  }
...
document.addEventListener('submit', async event => {
  ...
  if(event.target.id==='customer-form'){
    ...
    try{save();}catch{data=before;return;}
    const exportContact=fd.get('savePhoneContact')==='on';
    if(fromVisit){beginClientVisit(savedId,event.target.querySelector('[type="submit"]'),exportContact?()=>phoneContactModal(savedId):null);return;}
    modal=null;screen='customers';filter='All';query='';render();if(exportContact)phoneContactModal(savedId);else toast(existing?'Client details updated':'New client added');
  }
});
...
function updatePdfRecipient() {
  ...
  const phone=document.getElementById('pdf-recipient-phone');
  if(phone)phone.value=client?.phone||'';
  const copyPhone=document.querySelector('[data-action="copy-pdf-phone"]');
...
function openPdfShare(customerId='',taskId='') {
  ...
  modal='<div class="modal-backdrop" data-action="close-modal"><section class="modal" id="pdf-share-panel" role="dialog" aria-modal="true" aria-label="Send price-list PDF">'
    ...
    +'<div class="form-group"><label for="pdf-client">Client (optional)</label><select class="field" id="pdf-client"><option value="">Choose recipient in WhatsApp or email</option>'
    ...
    +'<button class="btn btn-primary btn-block" data-action="share-pdf-whatsapp" disabled>Share PDF via WhatsApp</button>'
    +'<p class="modal-hint">In your phone’s sharing menu choose WhatsApp (or WhatsApp Business), select the client and confirm Send. The actual PDF is included, not a website link. FieldFlow cannot choose the chat or verify delivery. If WhatsApp is not offered, download the PDF below and attach it as a Document in your chat.</p>'
    +'<button class="btn btn-secondary btn-block" data-action="share-pdf" disabled>'+icon('mail')+' Share PDF attachment</button>'
    ...
    +'<p class="modal-hint">If sharing is unavailable: download the PDF and attach it as a Document in WhatsApp, or open an email draft below and attach the file manually.</p>'
...
async function preparePdfShare() {
  ...
  const shareButtons=panel.querySelectorAll('[data-action="share-pdf"],[data-action="share-pdf-whatsapp"]');
  ...
  try {
    ...
    shareButtons.forEach(button=>button.disabled=!supported);
...
function emailReport(){
  let range;try{range=dateRange(reportPeriod,reportCustomStart,reportCustomEnd);}catch{range=dateRange('week');}
  const {visits,trips,tasksDue,tasksCompleted,minutes,undatedCompleted}=activityMetrics(data,range);
  const km=trips.reduce((sum,t)=>sum+Number(t.distanceKm||0),0),claim=trips.reduce((sum,t)=>sum+Number(t.reimbursement||0),0);
  const pipeline=data.customerWines.filter(item=>PIPELINE_STATUSES.includes(item.status)),windows=cycleReminders(),priorityWindows=windows.filter(item=>item.state==='open'||item.state==='due');
  const body=`Field activity — ${range.label}\n\nSELECTED PERIOD\nVisits: ${visits.length}\nCustomers seen: ${new Set(visits.map(v=>v.customerId)).size}\nTime in trade: ${formatTradeTime(minutes)}\nOpen follow-ups due in period: ${tasksDue.filter(t=>!t.done).length}\nFollow-ups completed in period: ${tasksCompleted.length}\nBusiness travel: ${km.toFixed(1)} km across ${trips.length} trips\nReimbursement claim: ${currency(claim)}\n\nCURRENT POSITION (not period-filtered)\nOpen follow-ups: ${data.tasks.filter(t=>!t.done).length}\nListing windows open or approaching: ${priorityWindows.length}\nExpired listing windows needing review: ${windows.filter(t=>t.state==='review').length}\nFuture listing windows: ${windows.filter(t=>t.state==='upcoming').length}\nWine pipeline: ${pipeline.length}\nConfirmed active listings: ${activeListings().length}\nOpen opportunity estimate (not sales): ${currency(data.customers.reduce((s,c)=>s+c.value,0))}${undatedCompleted?'\n\nData note: '+undatedCompleted+' completed follow-up(s) across all history have no valid completion date; excluded from period totals.':''}`;
  window.location.href=`mailto:?subject=${encodeURIComponent(`Field sales activity — ${range.label}`)}&body=${encodeURIComponent(body)}`;
}
...
function beginClientVisit(id,button,onStarted=null){
